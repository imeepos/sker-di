  ...
  resolve(): T {
    try {
      ...
      if (result === undefined || result === null) {
        throw new Error('无效的forwardRef: forwardRef函数返回了undefined或null');
      }
      return result;
    } catch (error) {
 ...
 * @returns 解析后的实际令牌
 */
export function resolveForwardRefCached<T>(tokenOrForwardRef: T | ForwardRef<T>): T {
  if (isForwardRef(tokenOrForwardRef)) {
    // 检查缓存
 ...
 * @returns 解析后的数组
 */
export function resolveForwardRefsInArray<T>(items: (T | ForwardRef<T>)[]): T[] {
 ...
 * @returns 解析后的依赖数组
 */
export function resolveForwardRefsInDeps(deps: any[] | undefined): any[] | undefined {
  if (!deps) {
    return deps;
  }
