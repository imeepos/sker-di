...
describe('根注入器单例测试', () => {
  ...
  describe('单例行为', () => {
    it('应该只允许创建一个根注入器', () => {
      // 第一次创建应该成功
      const rootInjector1 = injectorRegistry.createRootInjector([
    ...
    it('getRootInjector 应该返回已创建的根注入器', () => {
      ...
      // 获取应该返回同一个实例
      const retrievedInjector = injectorRegistry.getRootInjector();
    ...
    it('resetRootInjector 应该允许重新创建根注入器', () => {
      ...
      expect(injectorRegistry.getRootInjector()).toBe(rootInjector1);
  ...
  describe('与其他注入器的集成', () => {
    ...
    it('如果没有根注入器，平台注入器应该自动创建根注入器', () => {
      ...
      // 根注入器应该已经被自动创建
      const rootInjector = injectorRegistry.getRootInjector();
      expect(rootInjector).not.toBeNull();
      expect(platformInjector.parent).toBe(rootInjector);
    });
  ...
  describe('错误处理', () => {
    ...
    it('应该自动创建根注入器当创建平台注入器时', () => {
      expect(injectorRegistry.getRootInjector()).toBeNull();

      // 创建平台注入器应该自动创建根注入器
      const platformInjector = injectorRegistry.createPlatformInjector();
      expect(injectorRegistry.getRootInjector()).not.toBeNull();
