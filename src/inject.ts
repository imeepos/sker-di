...
import { InternalInjectFlags, convertInjectOptionsToFlags } from './internal-inject-flags';
...
export function getInjectMetadata(target: any): any[] | undefined {
  // 🚀 优先获取新的统一元数据
  const unifiedMetadata: InjectMetadata[] | undefined = Reflect.getMetadata(INJECT_METADATA_KEY, target);

  // 向后兼容：获取旧的分离元数据
  const legacyTokens: any[] | undefined = Reflect.getMetadata(LEGACY_INJECT_METADATA_KEY, target);
  ...
  for (let i = 0; i < maxLength; i++) {
    // 🚀 优先使用新的统一元数据
    if (unifiedMetadata && unifiedMetadata[i] && unifiedMetadata[i].token !== undefined) {
 ...
 * @returns 统一的注入元数据数组
 */
export function getUnifiedInjectMetadata(target: any): InjectMetadata[] | undefined {
  // 获取新的统一元数据
  const unifiedMetadata: InjectMetadata[] | undefined = Reflect.getMetadata(INJECT_METADATA_KEY, target);
  ...
  const legacyTokens: any[] | undefined = Reflect.getMetadata(LEGACY_INJECT_METADATA_KEY, target);
  const legacyOptions: InjectOptions[] | undefined = Reflect.getMetadata(LEGACY_INJECT_OPTIONS_METADATA_KEY, target);
  const paramTypes: any[] | undefined = Reflect.getMetadata(PARAM_TYPES_KEY, target);
