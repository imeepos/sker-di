 ...
 * @returns 组合后的标志值
 */
export function combineInjectFlags(...flags: InternalInjectFlags[]): InternalInjectFlags {
  return flags.reduce((combined, flag) => combined | flag, InternalInjectFlags.Default);
}
 ...
 * @returns 是否包含目标标志
 */
export function hasFlag(flags: InternalInjectFlags, targetFlag: InternalInjectFlags): boolean {
  // 对于 Default 标志的特殊处理
 ...
 * @returns 对应的注入选项对象
 */
export function convertFlagsToInjectOptions(flags: InternalInjectFlags): InjectOptions {
