...
function validateParameterDecoratorArgs<T>(
  ...
  // 验证参数索引
  if (typeof parameterIndex !== 'number' || parameterIndex < 0 || !Number.isInteger(parameterIndex)) {
    throw new Error(
...
export function Optional<T>(token: InjectionTokenType<T>): ParameterDecorator {
  return function (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) {
    // 🔍 验证参数
    validateParameterDecoratorArgs(token, target, propertyKey, parameterIndex, 'Optional');
...
export function Self<T>(token: InjectionTokenType<T>): ParameterDecorator {
  return function (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) {
    // 🔍 验证参数
    validateParameterDecoratorArgs(token, target, propertyKey, parameterIndex, 'Self');
...
export function SkipSelf<T>(token: InjectionTokenType<T>): ParameterDecorator {
  return function (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) {
    // 🔍 验证参数
    validateParameterDecoratorArgs(token, target, propertyKey, parameterIndex, 'SkipSelf');
...
export function Host<T>(token: InjectionTokenType<T>): ParameterDecorator {
  return function (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) {
    // 🔍 验证参数
    validateParameterDecoratorArgs(token, target, propertyKey, parameterIndex, 'Host');
